...
namespace Leo.Poulet.FeatureMatching
{
   ...
   {
       public async Task<IList<ObjectDetectionResult>> DetectObjectInScenesAsync(
           byte[] objectImageData, IList<byte[]> imagesSceneData)
       {
           // Liste des tâches parallèles
           var tasks = imagesSceneData.Select(imageSceneData =>
               Task.Run(() => DetectObjectInScene(objectImageData, imageSceneData))
           ).ToList();
